...
def verify( fully_respon=() ):
    ...
    try:
        ...
        if verify_respon.status_code == 200:
            ...
            verify_respon.close()
            if bool(fully_respon) is False:
                return verify_result_dict["result"]["status"]
            elif bool(fully_respon) is True:
...
def database_record_ipv4( fully_output=() ):
    # Read configuration
    ...
    try:
        ...
        if record_4_respon.status_code == 200:
            ...
            if bool(fully_output) is False:
                # Get IP address from dictionary
                return record_4_dict["result"]["content"]
            elif bool(fully_output) is True:
                # Return dictionary
                return record_4_dict
        else:
            record_4_respon.close()
...
def database_record_ipv6( fully_output=() ):
    # Read configuration
    record_config = configuration(update_config=False)
    # Get value
    zone_id = record_config["zone"]["zone_id"]
